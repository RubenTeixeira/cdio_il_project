...
public class GraphAlgorithms {
    ...
    {
        if (!qdfs.contains(vOrig.getElement()))
        {
        ...
        {
            Vertex<V, E> vAdj = g.opposite(vOrig, edge);
            int vKey = vAdj.getKey();
            if (visited[vKey] == false)
            {
                qdfs.add(vAdj.getElement());
    ...
    {
        Vertex<V, E> vertex = g.getVertex(vInf);
        if (vertex == null)
        {
            return null;
        }
    ...
    private static <V, E> void allPaths(Graph<V, E> g, Vertex<V, E> vOrig, Vertex<V, E> vDest,
            boolean[] visited, LinkedList<V> path, ArrayList<Deque<V>> paths)
    {
        ...
        for (Edge<V, E> edge : g.outgoingEdges(vOrig))
        {
            ...
            if (vAdj.equals(vDest))
            {
                ...
                paths.add(revPath((Deque<V>) path.clone()));
                path.pop();
    ...
    {
        Vertex<V, E> vOrigin = g.getVertex(voInf);
        Vertex<V, E> vDestiny = g.getVertex(vdInf);

        if (vOrigin == null || vDestiny == null)
        {
            return null;
        }
        ...
        LinkedList<V> qdfs = new LinkedList<>();
        boolean[] visited = new boolean[g.numVertices()];
        allPaths(g, vOrigin, vDestiny, visited, qdfs, paths);
        return paths;
    }
            ...
            boolean[] visited, int[] pathKeys, double[] dist)
    {
        ...
        LinkedList<Vertex<V, E>> path = new LinkedList<>();
        path.push(vOrig);
        dist[vOrig.getKey()] = 0;
        visited[vOrig.getKey()] = true;
        ...
        while (!path.isEmpty())
        {
            Vertex<V, E> remove = path.remove();
            for (Edge<V, E> edge : g.outgoingEdges(remove))
            {
                Vertex<V, E> vAdj = g.opposite(remove, edge);
                //double diste = dist[vAdj.getKey()] + g.getEdge(vOrig, vAdj).getWeight();
                //alt := dist[u] + dist_between(u, v)
                if (dist[vAdj.getKey()] == Double.MAX_VALUE || dist[vAdj.getKey()] > dist[remove.getKey()] + g.getEdge(remove, vAdj).getWeight())
                {
                    dist[vAdj.getKey()] = dist[remove.getKey()] + g.getEdge(remove, vAdj).getWeight();
                    visited[vAdj.getKey()] = true;
                    pathKeys[vAdj.getKey()] = remove.getKey();
                    path.push(vAdj);
                }
